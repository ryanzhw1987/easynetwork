...
IOBuffer::IOBuffer(unsigned int size/*=BUFFER_SIZE*/, unsigned int max_size/*=MAX_BUFFER_SIZE*/):m_max_size(max_size)
{
	m_buffer = NULL;
	m_buffer_size = 0;
...
char *IOBuffer::write_open(unsigned int size)
{
	...
	while(new_size-m_data_size<size && new_size<m_max_size)  //按原空间2倍扩展
		new_size <<= 1;
