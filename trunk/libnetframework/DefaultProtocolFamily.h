...
public:
	...
	//body_length:协议头长度
	bool encode(IOBuffer *io_buffer, int body_length);

	//解码协议头.成功返回true,失败返回false
	//io_buffer:待解码的协议头数据
	//body_length:解码后返回的协议体长度
	bool decode(IOBuffer *io_buffer, int &body_length);
public:
...
inline
bool DefaultProtocolHeader::encode(IOBuffer *io_buffer, int body_length)
{
	assert(io_buffer != NULL);
	int header_length = get_header_length();
	char *buf = io_buffer->write_open(header_length);
	...
	*(int*)buf = htonl(body_length);
	...
	io_buffer->write_close(header_length);
...
inline
bool DefaultProtocolHeader::decode(IOBuffer *io_buffer, int &body_length)
{
	assert(io_buffer != NULL);
	int header_length = get_header_length();
	unsigned int size=0;
	char *buf = io_buffer->read_open(size);

	if(buf==NULL || size<header_length)
	{
	...
	io_buffer->read_close(header_length);
...
public:
	...
	virtual Protocol* create_protocol(int protocol_type, int sequence=-1);
private:
	int m_magic_num;
};
//////////////////////////
...
Protocol* DefaultProtocolFamily::create_protocol(int protocol_type, int sequence/*=-1*/)
{
	...
	Protocol *protocol = ProtocolFamily::create_protocol(header);
	if(protocol != NULL)
	{
