...
int Protocol::encode(IOBuffer *io_buffer)
{
	...
	//头部的偏移位置
	unsigned int header_offset = io_buffer->get_size();

	//写入头部数据(没有真正写入数据,只是为头部预留空间)
	if(io_buffer->write_open(header_size) == NULL) //没有内存
		return -1;
	if(io_buffer->write_close(header_size) == false)
		return -1;
	...
	{
		result = io_buffer->truncate(header_size);
		assert(result == true);
		return -1;
	}
	//encode_body的时候可能导致数据迁移,需要重新定位header_buffer的位置.
	char *header_buffer = io_buffer->seek(header_offset, header_size);
	...
	{
		result = io_buffer->truncate(header_size+body_size);
		assert(result == true);
		return -1;
	}
	return 0;
